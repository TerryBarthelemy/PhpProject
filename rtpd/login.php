...
include __DIR__ .'/model/functions.php';
    ...
    if(isPostRequest()){
        ...
        $userCredentials = getLogin($email, $pwd)[0];
        var_dump($userCredentials);
    

        if($userCredentials["job_title"] == "Admin")
        {
            if($userCredentials['email'] == $email && $userCredentials['pwd'] == $pwd)
            {
                $_SESSION['admin_login'] = "TRUE";
                $_SESSION['user_id'] = $userCredentials['employee_id'];
                header('location: admin_announcements.php');
            }
        }

        else if($userCredentials["job_title"] == "Detective")
        {
            if($userCredentials['email'] == $email && $userCredentials['pwd'] == $pwd)
            {
